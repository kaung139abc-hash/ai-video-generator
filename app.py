...
async def create_project(
    ...
    plan = gemini_plan(novel, max_scenes)

    if not plan:
        plan = basic_plan(novel, max_scenes)
...
def get_project(project_id: str):
    ...
    return {
        "state": load_json(folder / "state.json"),
...
<div class="container">
    ...
    <div class="card">
        <h3>📖 ဝတ္ထု</h3>
        <textarea id="novel" placeholder="ဝတ္ထုတစ်ပုဒ်လုံး ဒီမှာထည့်ပါ..."></textarea>
        <br><br>
        <label>Scene အများဆုံး</label><br>
        <input id="scenes" type="number" value="8" min="1" max="30" style="padding:8px; width:100px; border-radius:8px; background:#090c12; color:#fff; border:1px solid #30394a; margin-top:5px;">
        <br>
        <button onclick="createProject()">🚀 Create Movie Project</button>
    </div>
    <div id="result"></div>
</div>
...
async function createProject(){
    ...
    formData.append("max_scenes", scenes);

    const resultDiv = document.getElementById("result");
    resultDiv.innerHTML = "<p class='small'>ပရောဂျက်ကို ဖန်တီးနေပါပြီ...</p>";
    ...
    try {
        ...
        if(response.ok){
            let html = `<div class='card'><h3>✨ ဖန်တီးပြီးစီးပါပြီ (ID: ${data.id})</h3>`;
            html += `<h4>ခေါင်းစဉ်: ${data.plan.title}</h4>`;
            
            data.plan.scenes.forEach(scene => {
                html += `<div class='scene'>
                    <strong>${scene.title}</strong><p>${scene.summary}</p>
                    <p class='small'>Visual Prompt: ${scene.visual_prompt}</p>
                </div>`;
            });
            html += `</div>`;
            resultDiv.innerHTML = html;
        } else {
            resultDiv.innerHTML = `<p style="color:red;">Error: ${data.error || "မအောင်မြင်ပါ"}</p>`;
        }
    } catch(err) {
        resultDiv.innerHTML = `<p style="color:red;">Network Error ဖြစ်ပွားသည်</p>`;
    }
}
</script>
